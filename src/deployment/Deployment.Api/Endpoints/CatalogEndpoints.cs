...
public static class CatalogEndpoints
{
    ...
    public static IEndpointRouteBuilder MapEnvironmentEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        {
            var cmd = new CreateEnvironmentCommand(body.Name, body.GcpProject, body.Region, body.GarRepository, body.KubernetesContext, body.KubernetesNamespace, body.IsProtected);
            return await EndpointHelpers.ValidateAndRun(v, cmd, ct, async () =>
            {
                var dto = await h.HandleAsync(cmd, ct);
        ...
        {
            var cmd = new UpdateEnvironmentCommand(id, body.Name, body.GcpProject, body.Region, body.GarRepository, body.KubernetesContext, body.KubernetesNamespace, body.IsProtected);
            return await EndpointHelpers.ValidateAndRun(v, cmd, ct, async () => { await h.HandleAsync(cmd, ct); return Results.NoContent(); });
        });
