...
public sealed record AspireApplicationDto(
    ...
    RolloutStrategyDto Strategy = RolloutStrategyDto.Direct,
    PromotionModeDto PromotionMode = PromotionModeDto.Automatic,
    string? ActiveSlot = null);

public sealed record CreateAspireApplicationRequest(string Name, string? Description, Guid EnvironmentId, string ManifestSource, string? Version, string? SourceKey = null, string? MainBranch = null, RolloutStrategyDto Strategy = RolloutStrategyDto.Direct, PromotionModeDto PromotionMode = PromotionModeDto.Automatic);
public sealed record UpdateAspireApplicationRequest(string Name, string? Description, Guid EnvironmentId, string ManifestSource, string? Version, string? SourceKey = null, string? MainBranch = null, RolloutStrategyDto Strategy = RolloutStrategyDto.Direct, PromotionModeDto PromotionMode = PromotionModeDto.Automatic);
...
public enum AspireRunStatusDto { Pending = 0, Running = 1, Succeeded = 2, Failed = 3, AwaitingApproval = 4, Rejected = 5, AwaitingPromotion = 6, RolledBack = 7 }
...
/// <summary>Promote / roll back a blue-green run awaiting promotion.</summary>
public sealed record PromoteAspireRunRequest(string? PromotedBy);
public sealed record RollbackAspireRunRequest(string? RolledBackBy, string? Reason);
