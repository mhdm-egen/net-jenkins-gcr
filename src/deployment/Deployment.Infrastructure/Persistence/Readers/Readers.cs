    ...
    private async Task<List<DeploymentMappingDto>> MapAsync(IReadOnlyList<Domain.Mappings.DeploymentMapping> mappings, CancellationToken ct)
    {
        ...
        return mappings
            ...
            .Select(m => new DeploymentMappingDto(
                ...
                m.Kubernetes == null ? null : new KubernetesSpecDto(m.Kubernetes.DeploymentName, m.Kubernetes.ContainerPort, m.Kubernetes.Replicas, m.Kubernetes.EnvVars, m.Kubernetes.ImagePullSecret, m.Kubernetes.CreateService, (RolloutStrategyDto)(int)m.Kubernetes.Strategy, (PromotionModeDto)(int)m.Kubernetes.PromotionMode),
                m.AutoDeploy,
