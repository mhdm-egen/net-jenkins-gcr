...
namespace Jenkins.WebUI.Services.Sca;
...
public static class SbomReader
{
    ...
    private static (bool Present, IReadOnlyList<SbomVulnerability> Items) ParseVulnerabilities(JsonElement root)
    {
        ...
        foreach (var v in arr.EnumerateArray())
        {
            ...
            list.Add(new SbomVulnerability(
                ...
                Source:      TryGetObject(v, "source") is { } s ? TryGetString(s, "name") : null,
    ...
    {
        if (!v.TryGetProperty("references", out var arr) || arr.ValueKind != JsonValueKind.Array)
            return Array.Empty<SbomVulnReference>();
        ...
        foreach (var entry in arr.EnumerateArray())
        {
            ...
            // CycloneDX nests the URL under source.url sometimes — fall back if needed.
