        ...
        stage('Deploy to Cloud Run') {
            steps {
                script {
                    ...
                    println "${image}"
                    def cmd = "gcloud run deploy ${params.GCR_APPHOST_SERVICE} --image=${image} --project=${params.GCP_PROJECT_ID} --region=${params.GCR_REGION} --service-account=${RUNTIME_SA} --platform=managed --allow-unauthenticated --port=8080 --memory=512Mi --cpu=1 --min-instances=0 --max-instances=1"
                    println "${cmd}"
                    sh "${cmd}"
                }
            }
